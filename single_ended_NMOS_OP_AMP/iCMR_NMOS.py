...
command1 = "scp -r vluz@193.136.221.60:/home/vluz/ICMR_NMOS_pair.csv /home/vasco/Desktop/circuit_design_plot_scripts/single_ended_NMOS_OP_AMP"


current_val = current_val/1000000
subprocess.run(command1, shell=True)
script_dir = os.path.dirname(__file__)
file = os.path.join(script_dir, "ICMR_NMOS_pair.csv")
...
interception_df = pd.DataFrame(interception.items(), columns=['Corner', 'Minimum input voltage'])
...
plt.savefig(os.path.join(script_dir,"Minimum_input_voltage_table.jpg"), bbox_inches='tight', pad_inches=0.05, format='jpg')
#plt.show()
...
plt.xlabel("input voltage", fontsize=12, weight='bold')  # x-axis label with units
...
plt.savefig(os.path.join(script_dir, 'Minimum_input_voltage.jpg'), format='jpg')
#plt.show()